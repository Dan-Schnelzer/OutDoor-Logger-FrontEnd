    ...
    <nav>
      ...
      <router-link :to="{ name: 'view-hiking-logs' }" class="nav-button">
        ...
        Create New Scouting Report</router-link>
          ...
          <router-link id="edit" class="nav-button" :to="{ name:'edit-scout-log', params: {id: scoutLog.scoutReportID}  }">
             EDIT SCOUT <br> * LOG *   </router-link>
    </nav>
   <article id="form-background">
          <!-- <div id="fishLog"> -->
           <div id="headLine">
         <h1 class="logLocation">{{  scoutLog.reportLocation }} </h1>
                <p class="logDate">{{ scoutLog.reportDate }}</p>
           </div>
           <div id="imageBig">
              <p class="imageURL">{{  scoutLog.imageURL }} :  images  </p>
              <!-- ***** this is what we will figure out later- loading the images here ****** -->
           </div>
          <div id="rightSide">
         <p class="scoutLogId"> Scout Report Id : {{ scoutLog.scoutReportID }} </p>
        <p class="logProps"> Time of Day : </p>
        <p class="logData"> {{  scoutLog.reportTime }} </p>
        <p class="logProps"> Weather Conditions : </p>
        <p class="logData" >{{  scoutLog.weather }}</p>
          </div>
          <div id="logDesc">
        <p id="logDescription"> Scout Report Description : </p>
        <p class="logData">  {{  scoutLog.scoutDescription }}</p>
          <!-- </div> -->
...
export default {
 ...
 created(){
    this.$store.dispatch("LOAD_SCOUT_LOG", this.$route.params.id);
  },
  computed:{
      scoutLog(){
          return this.$store.state.scoutLog;
      },
  }
